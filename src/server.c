...
int init_soket(IPaddress *ip, TCPsocket *tcpsock, SDLNet_SocketSet *set){
	...
	SDLNet_ResolveHost(ip, "127.0.0.2", 27015);
	...
	if(SDLNet_ResolveHost(ip, NULL, 27015)==-1) {
...
int game_loop(char* msg, int mypole[][10], int pole[][10]){
	switch (choose){
		case 1:
			...
			if (numready){
				if (SDLNet_SocketReady(new_tcpsock)) {
					result = SDLNet_TCP_Recv(new_tcpsock, msg, 3);
					if(result <= 0) {
						...
						if (choose == 0)
							printf("MISS IN [%c] [%c]\n", msg[1], msg[2]);
						result = SDLNet_TCP_Send(new_tcpsock, msg, 3);
						if( result < 3 ) {
